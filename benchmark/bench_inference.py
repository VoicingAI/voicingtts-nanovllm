"""Benchmark VoicingTTS inference throughput/latency.

Run (recommended via uv):
  uv run python benchmark/bench_inference.py --model ~/voicingtts --concurrency 4 --iters 5 --warmup 1
...
def _maybe_read_sample_rate(model: str) -> int | None:
    ...
    # VoicingTTSConfig.audio_vae_config.sample_rate default is 16000.
...
async def async_main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Benchmark VoicingTTS inference speed")
    ...
    try:
        # Async mode: from_pretrained returns AsyncVoicingTTSServerPool.
