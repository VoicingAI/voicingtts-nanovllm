...
VOICINGTTS_PROJ_LORA_TARGETS = ("enc_to_lm_proj", "lm_to_dit_proj", "res_to_dit_proj")
VOICINGTTS2_PROJ_LORA_TARGETS = (*VOICINGTTS_PROJ_LORA_TARGETS, "fusion_concat_proj")
...
def build_lora_config(model: str, *, max_loras: int, max_lora_rank: int) -> Any:
    ...
    if architecture == "voicingtts":
        from voicingtts_nanovllm.models.voicingtts.config import LoRAConfig
    elif architecture == "voicingtts2":
        from voicingtts_nanovllm.models.voicingtts2.config import LoRAConfig
    else:
...
def _proj_targets_for_architecture(architecture: str) -> tuple[str, ...]:
    if architecture == "voicingtts":
        return VOICINGTTS_PROJ_LORA_TARGETS
    if architecture == "voicingtts2":
        return VOICINGTTS2_PROJ_LORA_TARGETS
    raise ValueError(f"Unsupported model architecture for LoRA: {architecture}")
