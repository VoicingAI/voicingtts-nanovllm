"""Benchmark VoicingTTS TTFB under fixed request rate (open-loop).
...
async def async_main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Benchmark VoicingTTS TTFB under fixed RPS (open-loop)")
    p.add_argument(
        "--url",
        default=None,
        help=(
    ...
    server_pool: AsyncVoicingTTSServerPool | None = None
    if args.url is None:
        ...
        server_pool = cast(
            "AsyncVoicingTTSServerPool",
            VoicingTTS.from_pretrained(
                model=args.model,
    ...
    if sample_rate is not None and sample_rate > 0:
        ...
        if audio_s:
            ...
            rtfs = [
                _rtf_excluding_ttfb(r.wall_s, r.ttfb_s, r.total_samples / float(sample_rate))
                for r in results
                if r.ok and r.wall_s is not None and r.ttfb_s is not None and r.total_samples > 0
            ]
