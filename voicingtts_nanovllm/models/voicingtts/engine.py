...
from voicingtts_nanovllm.models.voicingtts.runner import VoicingTTSRunner, RunnerTask, VoicingTTSPayload
from voicingtts_nanovllm.config import Config
from voicingtts_nanovllm.models.voicingtts.config import VoicingTTSConfig
from voicingtts_nanovllm.models.voicingtts.lora_loader import load_voicingtts_lora_checkpoint
from voicingtts_nanovllm.engine.sequence import Sequence
from dataclasses import dataclass
import numpy as np
from transformers import LlamaTokenizerFast
from voicingtts_nanovllm.models.voicingtts.utils import mask_multichar_chinese_tokens
import torch


@dataclass
class VoicingTTSSeqPayload:
    # [(T, P, D)]
...
class VoicingTTSEngine(LLMEngineBase):
    def __init__(self, config: Config[VoicingTTSConfig]):
        ...
        super().__init__(VoicingTTSRunner, config, config.tensor_parallel_size)

    def register_lora(self, name: str, path: str) -> int:
        payload = load_voicingtts_lora_checkpoint(path, tp_size=self.model_runner.world_size)
        return super().register_lora(name, payload)

    def preprocess_seq(self, seq: Sequence[VoicingTTSSeqPayload], is_prefill: bool) -> RunnerTask[VoicingTTSPayload]:
        if is_prefill:
    ...
    def postprocess_seq(self, seq: Sequence[VoicingTTSSeqPayload], outputs: dict, is_prefill: bool):
