from voicingtts_nanovllm.models.voicingtts.engine import (
    VoicingTTSEngine,
    VoicingTTSRunner,
    VoicingTTSConfig,
    Config,
)
from voicingtts_nanovllm.models.voicingtts.config import LoRAConfig
import os
...
class VoicingTTSServerImpl:
    def __init__(
        ...
        model_config = VoicingTTSConfig.model_validate_json(open(os.path.join(model_path, "config.json")).read())
        ...
        self.llm = VoicingTTSEngine(engine_config)

        # VoicingTTSRunner attaches VAE helpers; the base runner interface doesn't.
        model_runner = cast(VoicingTTSRunner, self.llm.model_runner)
...
def main_loop(queue_in: mp.Queue, queue_out: mp.Queue, args, kwargs):
    ...
    try:
        srv = VoicingTTSServerImpl(*args, **kwargs)
    except Exception:
        queue_out.put(
            {
                "type": "init_error",
...
class AsyncVoicingTTSServerPool:
    def __init__(
        self,
        model_path: str,
        inference_timesteps: int = 10,
    ...
    async def register_lora(self, name: str, path: str) -> RegisterLoRAResponse:
        ...
        registered_servers: list[AsyncVoicingTTSServer] = []
        try:
...
class SyncVoicingTTSServerPool:
    def __init__(
        self,
        model_path: str,
        inference_timesteps: int = 10,
        ...
        **kwargs,
        ...
        async def init_async_server_pool():
            return AsyncVoicingTTSServerPool(
                model_path=model_path,
                inference_timesteps=inference_timesteps,
                max_num_batched_tokens=max_num_batched_tokens,
                max_num_seqs=max_num_seqs,
    ...
    def generate(
        ...
        ``AsyncVoicingTTSServerPool.generate``.
