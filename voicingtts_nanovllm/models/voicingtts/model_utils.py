"""CPU-testable pure helpers extracted from models/voicingtts/model.py.
...
Extraction map (model.py → model_utils.py):
  ...
  apply_rotary_emb_tokens     — body of BackboneLongRoPE._apply_rotary_emb (lines 135-153)
  compute_longrope_scaling_factor — math from BackboneLongRoPE.__init__ (line 81)
  compute_longrope_freqs      — body of BackboneLongRoPE._set_cos_sin_cache (lines 90-107)
...
def apply_rotary_pos_emb(
    ...
    Equivalent to the backbone implementation.  Computation is done
...
def apply_rotary_emb_tokens(
    ...
    """Apply rotary embedding per token (BackboneLongRoPE token-level variant).

    Matches the math inside ``BackboneLongRoPE._apply_rotary_emb``.
...
def compute_longrope_scaling_factor(
    ...
    This reproduces the formula used in ``BackboneLongRoPE.__init__``:
...
def compute_longrope_freqs(
    ...
    Mirrors the body of ``BackboneLongRoPE._set_cos_sin_cache``.
