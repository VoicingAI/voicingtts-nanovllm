...
from voicingtts_nanovllm.layers.activation import SiluAndMul
from voicingtts_nanovllm.layers.attention import Attention
from voicingtts_nanovllm.layers.layernorm import RMSNorm
from voicingtts_nanovllm.layers.linear import (
    QKVParallelLinear,
    MergedColumnParallelLinear,
    RowParallelLinear,
)
from voicingtts_nanovllm.layers.lora import (
    LoRAQKVParallelLinear,
    LoRAMergedColumnParallelLinear,
    LoRARowParallelLinear,
    LoRALinear,
)
from voicingtts_nanovllm.layers.embed_head import VocabParallelEmbedding
import math
...
def apply_rotary_pos_emb(q, k, cos, sin, position_ids, unsqueeze_dim=1):
    ...
    This mirrors the backbone implementation.
    """
    orig_dtype = k.dtype
...
class BackboneLongRoPE(nn.Module):
    """LongRoPE implementation matching the backbone."""
    ...
    def _apply_rotary_emb(self, x: torch.Tensor, cos: torch.Tensor, sin: torch.Tensor) -> torch.Tensor:
        """Apply rotary embedding using the backbone's corrected math."""
...
def get_backbone_rope(
    ...
    """Get the backbone LongRoPE implementation"""
    rotary_emb = BackboneLongRoPE(
        head_size=head_size,
...
class BackboneAttention(nn.Module):
    def __init__(
        ...
        self.rotary_emb = get_backbone_rope(
...
class BackboneDecoderLayer(nn.Module):
    def __init__(
        ...
        # depth scaling, as the backbone does
...
class BackboneModel(nn.Module):
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                BackboneDecoderLayer(config, is_causal, lora_config, lora_domain=lora_domain)
...
class VoicingTTSLocDiT(nn.Module):
    """
    ...
    def __init__(
        ...
        self.decoder = BackboneModel(config, is_causal=False, lora_config=dit_lora_config, lora_domain=DIT_LORA_DOMAIN)
...
class VoicingTTSModel(nn.Module):
    ...
    def __init__(
        ...
        self.residual_lm = BackboneModel(residual_lm_config, lora_config=lm_lora_config)
        ...
        self.feat_encoder = VoicingTTSLocEnc(encoder_config, input_dim=config.feat_dim)
        ...
        self.feat_decoder = UnifiedCFM(
            ...
            estimator=VoicingTTSLocDiT(decoder_config, in_channels=config.feat_dim, lora_config=lora_config),
        )
