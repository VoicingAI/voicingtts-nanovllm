...
class VoicingTTS2ServerImpl:
    def __init__(
        ...
        model_config = VoicingTTS2Config.model_validate_json(open(os.path.join(model_path, "config.json")).read())
        ...
        self.llm = VoicingTTS2Engine(engine_config)
        model_runner = cast(VoicingTTS2Runner, self.llm.model_runner)
...
def main_loop(queue_in: mp.Queue, queue_out: mp.Queue, args, kwargs):
    ...
    try:
        srv = VoicingTTS2ServerImpl(*args, **kwargs)
    except Exception:
...
class AsyncVoicingTTS2ServerPool:
    def __init__(
        self,
        model_path: str,
        inference_timesteps: int = 10,
    ...
    async def register_lora(self, name: str, path: str) -> RegisterLoRAResponse:
        ...
        registered_servers: list[AsyncVoicingTTS2Server] = []
        try:
...
class SyncVoicingTTS2ServerPool:
    def __init__(
        self,
        model_path: str,
        inference_timesteps: int = 10,
        ...
        **kwargs,
        ...
        async def init_async_server_pool():
            return AsyncVoicingTTS2ServerPool(
                model_path=model_path,
                inference_timesteps=inference_timesteps,
                max_num_batched_tokens=max_num_batched_tokens,
                max_num_seqs=max_num_seqs,
