"""voicingtts_nanovllm.engine.llm_engine
...
- Request state: :class:`~voicingtts_nanovllm.engine.sequence.Sequence`
  (prompt, generated tokens, KV bookkeeping).
- Scheduling + KV allocation: :class:`~voicingtts_nanovllm.engine.scheduler.Scheduler`
  (batching policy) and :class:`~voicingtts_nanovllm.engine.block_manager.BlockManager`
  (KV block pool + prefix caching).
- GPU execution: :class:`~voicingtts_nanovllm.engine.model_runner.BaseModelRunner`
...
2) The model-specific engine converts each selected :class:`Sequence` into a
   :class:`~voicingtts_nanovllm.engine.model_runner.RunnerTask` via
   :meth:`preprocess_seq`.
...
See the model packages under ``voicingtts_nanovllm/models/*`` for concrete
implementations.

Concrete example: VoicingTTS
------------------------
``voicingtts_nanovllm/models/voicingtts/engine.py`` implements an audio generation loop
...
from voicingtts_nanovllm.engine.lora_manager import LoRAManager, LoRAModelPayload
from voicingtts_nanovllm.engine.sequence import Sequence
from voicingtts_nanovllm.engine.scheduler import Scheduler
from voicingtts_nanovllm.engine.model_runner import RunnerTask, BaseModelRunner, select_lora_payload_for_rank
import socket
import torch
...
class LLMEngineBase:
    ...
    def preprocess_seq(self, seq: Sequence, is_prefill: bool) -> RunnerTask:
        """Convert a scheduled :class:`~voicingtts_nanovllm.engine.sequence.Sequence` into a runner input.

        The engine loop is model-agnostic, so each model family must implement
        how to build a :class:`~voicingtts_nanovllm.engine.model_runner.RunnerTask`
        ...
        Concrete example: VoicingTTS
        ------------------------
        See ``voicingtts_nanovllm/models/voicingtts/engine.py``:
        ...
        - Decode (``is_prefill=False``):
          VoicingTTS sends only the last step (length 1) as inputs, and sets
    ...
    def postprocess_seq(self, seq: Sequence, outputs: dict, is_prefill: bool):
        """Merge runner outputs back into the :class:`~voicingtts_nanovllm.engine.sequence.Sequence`.
        ...
        - Do not allocate/free KV blocks here. KV lifecycle is owned by
          :class:`~voicingtts_nanovllm.engine.scheduler.Scheduler` /
          :class:`~voicingtts_nanovllm.engine.block_manager.BlockManager`.

        Concrete example: VoicingTTS
        ------------------------
        In ``voicingtts_nanovllm/models/voicingtts/engine.py``:
