"""voicingtts_nanovllm.engine.scheduler

This module implements the batching/scheduling policy for the inference runtime.

The scheduler owns two queues of :class:`~voicingtts_nanovllm.engine.sequence.Sequence`:
...
- KV-cache capacity: enforced via :class:`~voicingtts_nanovllm.engine.block_manager.BlockManager`.
...
------------------------
VoicingTTS appends a newly generated latent patch as ``bytes`` into
``Sequence.token_ids`` in ``VoicingTTSEngine.postprocess_seq``. On the next step,
the scheduler will:
...
from voicingtts_nanovllm.engine.sequence import Sequence, SequenceStatus
from voicingtts_nanovllm.engine.block_manager import BlockManager
