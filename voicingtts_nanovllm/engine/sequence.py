"""voicingtts_nanovllm.engine.sequence
...
---------------------------
The engine operates in a tight loop (see :class:`~voicingtts_nanovllm.engine.llm_engine.LLMEngineBase`):

1) Users (or model-specific engines) construct a :class:`Sequence` from an input
   prompt and enqueue it into :class:`~voicingtts_nanovllm.engine.scheduler.Scheduler`.
2) The :class:`~voicingtts_nanovllm.engine.scheduler.Scheduler` transitions sequences
   between ``WAITING`` and ``RUNNING`` and decides which ones to execute next.
3) :class:`~voicingtts_nanovllm.engine.block_manager.BlockManager` allocates/free KV
   blocks and fills ``Sequence.block_table`` (physical block ids used by the
   Attention KV-cache).
4) The model-specific engine converts :class:`Sequence` ->
   :class:`~voicingtts_nanovllm.engine.model_runner.RunnerTask` (runner-friendly
...
- ``num_cached_tokens``: Count of prompt tokens whose KV state is reused from
  the prefix cache (see :mod:`voicingtts_nanovllm.engine.block_manager`).
...
------------------------
The VoicingTTS implementation under ``voicingtts_nanovllm/models/voicingtts`` uses
...
- Actual model inputs (text ids, audio features, masks, sampling params) live in
  ...
  ``VoicingTTSEngine.preprocess_seq`` / ``VoicingTTSRunner.run``.
