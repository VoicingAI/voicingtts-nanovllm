...
# voicingtts_nanovllm/__init__.py imports llm, which checks flash-attn on import.
...
class FakeServerPool:
    """CPU-safe fake for AsyncVoicingTTSServerPool used by lifespan."""
...
def app(monkeypatch, tmp_path):
    ...
    model_dir.mkdir()
    (model_dir / "config.json").write_text(json.dumps({"architecture": "voicingtts"}), encoding="utf-8")
