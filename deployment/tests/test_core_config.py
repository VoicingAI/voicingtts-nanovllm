...
def test_materialize_lora_config_defaults_are_architecture_aware():
    ...
    voicingtts = materialize_lora_config(raw, "voicingtts")
    assert voicingtts.enable_lm is True
    assert voicingtts.enable_dit is True
    assert voicingtts.enable_proj is True
    assert voicingtts.target_modules_lm == (
        "q_proj",
        "k_proj",
        "v_proj",
        "o_proj",
    ...
    assert voicingtts.target_proj_modules == ("enc_to_lm_proj", "lm_to_dit_proj", "res_to_dit_proj")

    voicingtts2 = materialize_lora_config(raw, "voicingtts2")
    assert voicingtts2.target_proj_modules == (
        "enc_to_lm_proj",
        "lm_to_dit_proj",
        "res_to_dit_proj",
        "fusion_concat_proj",
...
def test_materialize_lora_config_preserves_overrides():
    ...
    config = materialize_lora_config(raw, "voicingtts2")
...
def test_load_config_expands_user_paths(monkeypatch):
    ...
    monkeypatch.setenv("NANOVLLM_MODEL_PATH", "~/voicingtts")
    cfg = load_config()
    assert cfg.model_path == os.path.expanduser("~/voicingtts")
