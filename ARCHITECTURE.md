# voicingtts-nanovllm Architecture

This document explains the architecture of the `voicingtts_nanovllm/` package: what the major modules do, how they interact at runtime, and where to start when you are new to the codebase.
...
- [VoicingTTS Integration](#voicingtts-integration-model-specific-layer)
...
- A **concrete model integration** for **VoicingTTS** (audio generation) built on top of that runtime.

This is GPU-centric code. Many paths assume CUDA, FlashAttention (`flash-attn`), Triton, and NCCL.

## Mental Model (One Paragraph)

At runtime, a *request* is represented as a `Sequence` (prompt state + generated tokens + KV-cache mapping). The `Scheduler` batches sequences and enforces resource limits. The `BlockManager` allocates a shared KV-cache block pool and supports prefix caching via hashing. The engine (`LLMEngineBase`) runs an event loop: it schedules sequences, converts them into lightweight `RunnerTask` objects, calls the GPU runner (`BaseModelRunner`) to execute the model, then merges outputs back into each `Sequence` until they finish. VoicingTTS provides the model-specific glue: how to build inputs from text/audio, how to interpret outputs (latents + stop flag + waveform chunks), and how to expose this as an async streaming server.

## High-Level Component Diagram

```mermaid
flowchart TB
  U[User code] -->|VoicingTTS.from_pretrained| API[voicingtts_nanovllm/llm.py]
  API --> POOL[models/voicingtts/server.py<br/>AsyncVoicingTTSServerPool / SyncVoicingTTSServerPool]
  POOL --> S1[AsyncVoicingTTSServer<br/>1 per GPU device]
  S1 -->|mp.Queue RPC| IMPL[VoicingTTSServerImpl]
  IMPL --> ENG[VoicingTTSEngine<br/>models/voicingtts/engine.py]
  ...
  RUN --> MODEL[VoicingTTSModel<br/>models/voicingtts/model.py]
...
`voicingtts_nanovllm/` is split into three main layers:

1) **Public entrypoint**

- `voicingtts_nanovllm/__init__.py`: exports `VoicingTTS`.
- `voicingtts_nanovllm/llm.py`: user-facing factory (`VoicingTTS.from_pretrained`) that returns a server pool.
- `voicingtts_nanovllm/config.py`: generic runtime `Config` shared by the core engine.

2) **Model-agnostic runtime core** (`voicingtts_nanovllm/engine/`)
...
3) **Model integration** (`voicingtts_nanovllm/models/voicingtts/`)

- `models/voicingtts/config.py`: Pydantic model configs (VoicingTTSConfig, LoRAConfig, etc.).
- `models/voicingtts/model.py`: `VoicingTTSModel` definition (text LM + residual LM + local encoder + diffusion decoder + stop predictor).
- `models/voicingtts/runner.py`: `VoicingTTSRunner` (build tensors, set attention context, run model, decode to waveform chunk).
- `models/voicingtts/engine.py`: `VoicingTTSEngine` (create `Sequence`, implement preprocess/postprocess glue).
- `models/voicingtts/server.py`: multiprocessing server + async streaming API + LoRA methods.
- `models/voicingtts/utils.py`: tokenizer wrapper to split multi-char Chinese tokens.

Supporting utilities / layers:

- `voicingtts_nanovllm/utils/context.py`: global “attention context” (slot mapping, cu_seqlens, etc.) used by flash-attn kernels.
- `voicingtts_nanovllm/utils/loader.py`: load `.safetensors` weights and LoRA weights with name mapping.
- `voicingtts_nanovllm/layers/*`: tensor-parallel linear/embedding layers, flash-attn attention, LoRA layers, and AudioVAE.

## Core Runtime Concepts

### 1) Sequence (Request State)

File: `voicingtts_nanovllm/engine/sequence.py`
...
- `custom_payload` is model-specific state (for VoicingTTS: text tokens, audio features, masks, sampling controls, streaming buffers).
...
- Prefix caching hashes tokens in full KV blocks. VoicingTTS needs to hash both text ids (`int`) and audio latent patches (`bytes`), so the token stream supports both.

### 2) KV Cache as a Block Pool (+ Prefix Caching)

File: `voicingtts_nanovllm/engine/block_manager.py`
...
File: `voicingtts_nanovllm/engine/scheduler.py`
...
File: `voicingtts_nanovllm/engine/model_runner.py`
...
- `voicingtts_nanovllm/utils/context.py`
- `voicingtts_nanovllm/layers/attention.py`
...
Current code is not fully aligned yet: `voicingtts_nanovllm/models/voicingtts/server.py` still contains a server-side GPU transfer in `encode_latents()`. The ADR and standard document the intended boundary and the required cleanup direction.
...
## VoicingTTS Integration (Model-Specific Layer)

The VoicingTTS integration demonstrates how to plug a model into the runtime.

### VoicingTTSModel

File: `voicingtts_nanovllm/models/voicingtts/model.py`

`VoicingTTSModel` combines several parts:
...
- `latents`: predicted feature patch (for VoicingTTS, shaped like `[P, D]` for patch size `P` and feature dim `D`).
- `stop_flag`: whether generation should end.

### VoicingTTSEngine (Glue Code)

File: `voicingtts_nanovllm/models/voicingtts/engine.py`

`VoicingTTSEngine` subclasses `LLMEngineBase` and implements the two crucial adapters:
...
### VoicingTTSRunner (Execution + VAE Decode)

File: `voicingtts_nanovllm/models/voicingtts/runner.py`

`VoicingTTSRunner` is the concrete `BaseModelRunner` implementation:
...
- The waveform chunk sample rate is `VoicingTTSRunner.vae.sample_rate` (configured by `AudioVAEConfig.sample_rate`, default 16000).

### VoicingTTS Server (Multiprocessing + Async Streaming)

File: `voicingtts_nanovllm/models/voicingtts/server.py`

The server stack has three layers:

1) `VoicingTTSServerImpl`: owns the `VoicingTTSEngine` and provides methods like `add_request`, `step`, `encode_latents`.
2) `AsyncVoicingTTSServer`: runs `VoicingTTSServerImpl` in a spawned process and communicates through `mp.Queue` (RPC + stream messages).
3) `AsyncVoicingTTSServerPool`: manages one server per GPU device and load-balances requests.

Parallelism note:

- The current VoicingTTS server construction uses one process per GPU device (the server pool). The generic engine/runner supports tensor-parallel multi-process execution via `Config.tensor_parallel_size`, but `VoicingTTSServerImpl` currently builds `Config(...)` with the default `tensor_parallel_size=1`.
...
sequenceDiagram
  ...
  participant P as AsyncVoicingTTSServerPool
  participant S as AsyncVoicingTTSServer
  participant SP as Server process
  participant E as VoicingTTSEngine
...
File: `voicingtts_nanovllm/utils/loader.py`
...
- `voicingtts_nanovllm/models/voicingtts/config.py` (LoRAConfig)
- `voicingtts_nanovllm/layers/lora.py` (LoRA modules)
...
2) VoicingTTS engine converts each `Sequence` to a `RunnerTask` (slicing away cached prefix tokens during prefill).
...
5) VoicingTTS engine appends the new latent patch into `Sequence.token_ids`, updates request payload, and sets `seq.stoped` if needed.
...
1) `voicingtts_nanovllm/llm.py` (public entrypoint, server pool selection)
2) `voicingtts_nanovllm/models/voicingtts/server.py` (async streaming + process loop)
3) `voicingtts_nanovllm/models/voicingtts/engine.py` (how VoicingTTS maps requests to the runtime)
4) `voicingtts_nanovllm/engine/llm_engine.py` (generic orchestration)
5) `voicingtts_nanovllm/engine/scheduler.py` + `voicingtts_nanovllm/engine/block_manager.py` (batching + KV blocks + prefix caching)
6) `voicingtts_nanovllm/engine/model_runner.py` + `voicingtts_nanovllm/layers/attention.py` (context + flash-attn + KV writes)
7) `voicingtts_nanovllm/models/voicingtts/model.py` (the actual VoicingTTS model)

## Extending the System (Adding a New Model Family)

The runtime is intentionally model-agnostic. A new model family typically adds a new package under `voicingtts_nanovllm/models/<name>/` with:
