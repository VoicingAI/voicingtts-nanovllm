...
only the attributes your test exercises. From `tests/unit/test_voicingtts_engine_max_model_len.py:16-30`:
...
uv run diff-cover coverage.xml --compare-branch=origin/main --fail-under=80 \
  --include 'voicingtts_nanovllm/engine/**' \
  --include 'voicingtts_nanovllm/models/voicingtts/**' \
  --include 'voicingtts_nanovllm/lora/**'
```
