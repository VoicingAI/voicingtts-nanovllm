...
def _make_engine(max_model_len: int, token_count: int):
    """Create a VoicingTTS2Engine instance without heavy init."""

    from voicingtts_nanovllm.models.voicingtts2.engine import VoicingTTS2Engine

    e = VoicingTTS2Engine.__new__(VoicingTTS2Engine)
...
def test_postprocess_advances_seed_step_after_generated_latent():
    ...
    from voicingtts_nanovllm.models.voicingtts2.engine import VoicingTTS2Engine, VoicingTTS2SeqPayload
    ...
    class _Seq:
        ...
        def __init__(self):
            self.tokens = []
            self.custom_payload = VoicingTTS2SeqPayload(
                feats=[],
                text_tokens=[],
                feat_masks=[],
                generated_waveforms=[],
