"""Unit tests for voicingtts_nanovllm.models.voicingtts.server route/validation logic.

All tests run without a real GPU model.  The ``AsyncVoicingTTSServerPool`` is
...
``test_voicingtts_serverpool_lora_api.py``.

Tests that cover ``VoicingTTSServerImpl`` instance methods use a synthetic object
...
class _FakeServer:
    """Minimal fake for AsyncVoicingTTSServer used inside pool tests."""
...
# ---------------------------------------------------------------------------
# Helper: build a bare AsyncVoicingTTSServerPool
# ---------------------------------------------------------------------------


def _make_pool(servers):
    from voicingtts_nanovllm.models.voicingtts.server import AsyncVoicingTTSServerPool

    pool = object.__new__(AsyncVoicingTTSServerPool)
...
def test_gen_uuid_returns_hex_string():
    from voicingtts_nanovllm.models.voicingtts.server import gen_uuid
...
# ---------------------------------------------------------------------------
# VoicingTTSServerImpl – unit-level (no engine)
# ---------------------------------------------------------------------------
...
def _make_server_impl(feat_dim: int = 4, patch_size: int = 1):
    from voicingtts_nanovllm.models.voicingtts.server import VoicingTTSServerImpl

    srv = VoicingTTSServerImpl.__new__(VoicingTTSServerImpl)
...
def test_server_impl_register_and_list_and_unregister_lora():
    from voicingtts_nanovllm.models.voicingtts.server import LoRAInfo
...
# ---------------------------------------------------------------------------
# VoicingTTSServerImpl.add_request – validation paths
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# VoicingTTSServerImpl.encode_latents – resampling + stereo paths
# ---------------------------------------------------------------------------
...
def test_encode_latents_resamples_when_sr_mismatch(monkeypatch):
    ...
    monkeypatch.setattr("voicingtts_nanovllm.models.voicingtts.server.torchaudio.load", _fake_load)
    monkeypatch.setattr("voicingtts_nanovllm.models.voicingtts.server.torchaudio.functional.resample", _fake_resample)
...
# ---------------------------------------------------------------------------
# AsyncVoicingTTSServerPool – get_model_info empty-pool guard
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AsyncVoicingTTSServerPool – register_lora duplicate guard
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AsyncVoicingTTSServerPool – unregister_lora guards
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AsyncVoicingTTSServerPool – encode_latents delegates to min-load server
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AsyncVoicingTTSServerPool – add_prompt and remove_prompt
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AsyncVoicingTTSServerPool – generate with prompt_id validation
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AsyncVoicingTTSServerPool – wait_for_ready and stop delegate to servers
# ---------------------------------------------------------------------------
...
def test_async_server_rejects_unknown_kwargs_before_starting_process():
    from voicingtts_nanovllm.models.voicingtts.server import AsyncVoicingTTSServer

    with pytest.raises(ValueError, match="Unknown kwargs"):
        AsyncVoicingTTSServer("/fake/model", unsupported=True)
...
def test_async_server_pool_rejects_unknown_kwargs_before_starting_servers():
    ...
    with pytest.raises(ValueError, match="Unknown kwargs"):
        AsyncVoicingTTSServerPool("/fake/model", unsupported=True)


async def _async_server_forwards_control_plane_calls():
    from voicingtts_nanovllm.models.voicingtts.server import AsyncVoicingTTSServer

    server = object.__new__(AsyncVoicingTTSServer)
    calls = []
