...
def _make_engine(max_model_len: int, token_count: int):
    """Create a VoicingTTSEngine instance without heavy init.

    We bypass VoicingTTSEngine.__init__ because it loads tokenizer/model weights and
    initializes GPU runtime. For these unit tests we only need to exercise
    add_request()'s length checks.
    """

    from voicingtts_nanovllm.models.voicingtts.engine import VoicingTTSEngine

    e = VoicingTTSEngine.__new__(VoicingTTSEngine)
...
def test_postprocess_advances_seed_step_after_generated_latent():
    ...
    from voicingtts_nanovllm.models.voicingtts.engine import VoicingTTSEngine, VoicingTTSSeqPayload

    engine = VoicingTTSEngine.__new__(VoicingTTSEngine)
    engine.feat_dim = 2
    engine.patch_size = 4
    ...
    class _Seq:
        ...
        def __init__(self):
            self.tokens = []
            self.custom_payload = VoicingTTSSeqPayload(
                feats=[],
                text_tokens=[],
                feat_masks=[],
                generated_waveforms=[],
