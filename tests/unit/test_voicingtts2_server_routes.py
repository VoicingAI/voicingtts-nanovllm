"""Unit tests for voicingtts_nanovllm.models.voicingtts2.server route/validation logic.

All tests run without a real GPU model.  The ``AsyncVoicingTTS2ServerPool`` is
...
``test_voicingtts2_serverpool_lora_api.py``.
"""
...
class _FakeServer:
    """Minimal fake for AsyncVoicingTTS2Server used inside pool tests."""
...
# ---------------------------------------------------------------------------
# Helper: build a bare AsyncVoicingTTS2ServerPool
# ---------------------------------------------------------------------------


def _make_pool(servers):
    from voicingtts_nanovllm.models.voicingtts2.server import AsyncVoicingTTS2ServerPool

    pool = object.__new__(AsyncVoicingTTS2ServerPool)
...
def test_gen_uuid_returns_hex_string():
    from voicingtts_nanovllm.models.voicingtts2.server import gen_uuid
...
# ---------------------------------------------------------------------------
# VoicingTTS2ServerImpl – unit-level (no engine)
# ---------------------------------------------------------------------------
...
def _make_server_impl(feat_dim: int = 4, patch_size: int = 1):
    from voicingtts_nanovllm.models.voicingtts2.server import VoicingTTS2ServerImpl

    srv = VoicingTTS2ServerImpl.__new__(VoicingTTS2ServerImpl)
...
def test_server_impl_register_and_list_and_unregister_lora():
    from voicingtts_nanovllm.models.voicingtts2.server import LoRAInfo
...
# ---------------------------------------------------------------------------
# VoicingTTS2ServerImpl.add_request – validation paths
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# VoicingTTS2ServerImpl.encode_latents – stereo path
# ---------------------------------------------------------------------------
...
def test_encode_latents_converts_stereo_to_mono(monkeypatch):
    ...
    monkeypatch.setattr("voicingtts_nanovllm.models.voicingtts2.server.librosa.load", _fake_librosa_load)
...
# ---------------------------------------------------------------------------
# AsyncVoicingTTS2ServerPool – get_model_info empty-pool guard
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AsyncVoicingTTS2ServerPool – register_lora guards
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AsyncVoicingTTS2ServerPool – unregister_lora guards
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AsyncVoicingTTS2ServerPool – encode_latents routes to min-load server
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AsyncVoicingTTS2ServerPool – add_prompt and remove_prompt
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AsyncVoicingTTS2ServerPool – generate with prompt_id validation
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# AsyncVoicingTTS2ServerPool – wait_for_ready and stop
# ---------------------------------------------------------------------------
...
def test_async_server_rejects_unknown_kwargs_before_starting_process():
    from voicingtts_nanovllm.models.voicingtts2.server import AsyncVoicingTTS2Server

    with pytest.raises(ValueError, match="Unknown kwargs"):
        AsyncVoicingTTS2Server("/fake/model", unsupported=True)
...
def test_async_server_pool_rejects_unknown_kwargs_before_starting_servers():
    ...
    with pytest.raises(ValueError, match="Unknown kwargs"):
        AsyncVoicingTTS2ServerPool("/fake/model", unsupported=True)


async def _async_server_forwards_control_plane_calls():
    from voicingtts_nanovllm.models.voicingtts2.server import AsyncVoicingTTS2Server

    server = object.__new__(AsyncVoicingTTS2Server)
    calls = []
