...
def test_audio_vae_v2_exposes_encoder_and_decoder_chunk_sizes():
    from voicingtts_nanovllm.layers.audio_vae_v2 import AudioVAEConfigV2, AudioVAEV2
...
def test_audio_vae_v2_uses_fixed_batch_shaped_sr_idx():
    from voicingtts_nanovllm.layers.audio_vae_v2 import CausalDecoder
...
def test_voicingtts2_runner_prepares_fp16_decoder_and_folds_weight_norm():
    from torch import nn
    from torch.nn.utils import weight_norm

    from voicingtts_nanovllm.models.voicingtts2.runner import VoicingTTS2Runner

    runner = VoicingTTS2Runner.__new__(VoicingTTS2Runner)
...
def test_voicingtts2_engine_aligns_prompt_audio_with_encoder_chunk_size():
    ...
    engine = VoicingTTS2Engine.__new__(VoicingTTS2Engine)
    engine.patch_size = 4
...
def test_voicingtts2_runner_slices_decoded_waveform_with_decoder_chunk_size(monkeypatch):
    from voicingtts_nanovllm.models.voicingtts2.runner import VoicingTTS2Payload, VoicingTTS2Runner
    ...
    seq = type(
        ...
        (),
        {
            "custom_payload": VoicingTTS2Payload(
