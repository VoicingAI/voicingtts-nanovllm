"""Unit tests for voicingtts_nanovllm.models.voicingtts.runner and runner_utils.

All tests are CPU-only (no real GPU workers, no model weights).
The __new__ + attribute injection idiom is used (same as test_voicingtts_engine_max_model_len.py).
"""
...
def _make_payload(T=3, P=4, D=8, seed=None, seed_step=0, padding_decode=None, temperature=1.0, cfg_value=1.5):
    from voicingtts_nanovllm.models.voicingtts.runner import VoicingTTSPayload

    return VoicingTTSPayload(
...
def _make_runner(patch_size=4, feat_dim=8):
    from voicingtts_nanovllm.models.voicingtts.runner import VoicingTTSRunner

    r = VoicingTTSRunner.__new__(VoicingTTSRunner)
...
# ---------------------------------------------------------------------------
# VoicingTTSPayload dataclass
# ---------------------------------------------------------------------------


class TestVoicingTTSPayload:
    ...
    def test_field_assignment(self):
        ...
        p = VoicingTTSPayload(text_tokens=arr, temperature=0.7, cfg_value=2.0, seed=42, seed_step=3)
...
# ---------------------------------------------------------------------------
# VoicingTTSRunner shape methods (no GPU)
# ---------------------------------------------------------------------------


class TestVoicingTTSRunnerDummyMethods:
    ...
    def test_dit_lora_seq_len_offset(self):
        ...
        assert VoicingTTSRunner.dit_lora_seq_len_offset == 1
