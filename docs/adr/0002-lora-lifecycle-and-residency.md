...
- `voicingtts_nanovllm/models/voicingtts/server.py:574` and `voicingtts_nanovllm/models/voicingtts2/server.py:496` make `ServerPool` the public LoRA truth source for `register_lora`, `unregister_lora`, `list_loras`, and `generate(..., lora_name=...)`.
- `voicingtts_nanovllm/engine/llm_engine.py:120` keeps an engine-side `LoRAManager` for `name -> adapter_id`, request lifecycle callbacks, draining semantics, and scheduler admission checks.
- `voicingtts_nanovllm/engine/model_runner.py:170` keeps a runner-side `LoRARuntime` for rank-local payload registration, GPU slot residency, slot eviction, batch planning, and LoRA execution metadata.
- `voicingtts_nanovllm/engine/lora_manager.py:229` and `voicingtts_nanovllm/engine/lora_manager.py:245` now explicitly separate these two responsibilities into `LoRAManager` and `LoRARuntime`.
- `voicingtts_nanovllm/utils/context.py:18` and `voicingtts_nanovllm/layers/lora.py:146` define a dedicated LoRA runtime context that LoRA-capable layers consume independently from attention context.
- `voicingtts_nanovllm/engine/model_runner.py:555` to `voicingtts_nanovllm/engine/model_runner.py:766` show that LoRA execution must remain compatible with CUDA graph capture and replay.
...
- `voicingtts_nanovllm/models/voicingtts/server.py:574` rejects duplicate registration names before issuing per-server registration.
- `voicingtts_nanovllm/models/voicingtts/server.py:578` to `voicingtts_nanovllm/models/voicingtts/server.py:589` commit registration only after all servers succeed, and roll back already-registered servers on failure.
- `voicingtts_nanovllm/models/voicingtts/server.py:592` to `voicingtts_nanovllm/models/voicingtts/server.py:606` remove a LoRA from new-request visibility by moving it through `_draining_loras` before final removal from `_registered_loras`.
- `voicingtts_nanovllm/models/voicingtts/server.py:677` rejects `generate(..., lora_name=...)` when the name is absent or draining.

The same semantics are mirrored in `voicingtts_nanovllm/models/voicingtts2/server.py:496` to `voicingtts_nanovllm/models/voicingtts2/server.py:555`.

#### `Server`

`Server` is a transport and state-replica layer only.

- `voicingtts_nanovllm/models/voicingtts/server.py:162` to `voicingtts_nanovllm/models/voicingtts/server.py:171` forward register/unregister/list calls to the local engine.
...
- `voicingtts_nanovllm/engine/lora_manager.py:229` keeps `LoRAManager` focused on registration, lifecycle state, adapter identity, and admission semantics.
- `voicingtts_nanovllm/engine/llm_engine.py:136` to `voicingtts_nanovllm/engine/llm_engine.py:183` fan out register/unregister and sequence lifecycle events to both the engine manager and the runner runtime.
...
- `voicingtts_nanovllm/engine/lora_manager.py:245` keeps `LoRARuntime` responsible for runtime payload retention, slot assignment, LRU eviction, and batch-plan construction.
- `voicingtts_nanovllm/engine/model_runner.py:249` to `voicingtts_nanovllm/engine/model_runner.py:304` route all runner-side LoRA operations through `lora_runtime`.
...
- `voicingtts_nanovllm/models/voicingtts/engine.py:46` loads the checkpoint into model payloads before delegating to `LLMEngineBase.register_lora()`.
- `voicingtts_nanovllm/engine/llm_engine.py:136` validates and registers the payload across runner ranks, then stores the local adapter identity in the engine manager.
- `voicingtts_nanovllm/engine/model_runner.py:249` stores each rank-local payload in `LoRARuntime`.
...
The lifecycle is defined by `voicingtts_nanovllm/engine/lora_manager.py:12` and the transition methods in `voicingtts_nanovllm/engine/lora_manager.py:127` to `voicingtts_nanovllm/engine/lora_manager.py:188`.
...
- `voicingtts_nanovllm/engine/lora_manager.py:124` rejects new requests once the state becomes `DRAINING`.
- `voicingtts_nanovllm/engine/lora_manager.py:134` to `voicingtts_nanovllm/engine/lora_manager.py:139` move the entry to `DRAINING`, then remove it immediately only if no bound request remains.
...
- `voicingtts_nanovllm/engine/lora_manager.py:57` defines `LoRASlot` as `slot_id`, `adapter_id`, `resident_state`, and `last_used_ts`.
- `voicingtts_nanovllm/engine/lora_manager.py:332` to `voicingtts_nanovllm/engine/lora_manager.py:370` implement slot admission and eviction.
...
- `voicingtts_nanovllm/engine/llm_engine.py:166` delegates scheduler admission checks to `LoRAManager.can_schedule()`.
- `voicingtts_nanovllm/engine/lora_manager.py:190` to `voicingtts_nanovllm/engine/lora_manager.py:205` treat LoRA slot availability as a hard scheduling resource.
...
- `voicingtts_nanovllm/engine/model_runner.py:298` to `voicingtts_nanovllm/engine/model_runner.py:334` build `token_to_slot`, grouped token indices, active slot ids, slot offsets, and scratch space.
- `voicingtts_nanovllm/utils/context.py:18` to `voicingtts_nanovllm/utils/context.py:87` keep LoRA runtime metadata separate from attention metadata.
- `voicingtts_nanovllm/layers/lora.py:146` to `voicingtts_nanovllm/layers/lora.py:160` package LoRA runtime metadata for backend kernels.
...
- `voicingtts_nanovllm/engine/model_runner.py:555` to `voicingtts_nanovllm/engine/model_runner.py:686` preallocate fixed-capacity graph-visible LoRA tensors for decode graph capture.
- `voicingtts_nanovllm/engine/model_runner.py:703` to `voicingtts_nanovllm/engine/model_runner.py:760` replay graphs by mutating contents of existing buffers rather than allocating new graph-visible tensors.
- `voicingtts_nanovllm/engine/model_runner.py:277` to `voicingtts_nanovllm/engine/model_runner.py:296` perform LoRA slot H2D transfer only inside the runner.
...
- `voicingtts_nanovllm/engine/llm_engine.py:139` allocates a stable adapter id once, then broadcasts registration to all ranks.
- `voicingtts_nanovllm/engine/model_runner.py:99` selects rank-local payload shards during runner registration.
